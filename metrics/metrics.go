...
type descriptor struct {
	...
	kind     legacyKind
	isFloat  bool   // single gauges only: render value as float
...
// Gatherer exposes the underlying prometheus.Gatherer (e.g. for tests).
...
func LiveMetrics(r *Registry) *Metrics {
	return &Metrics{
		...
		ConnTimeoutCounter:      r.registerCounter("conn.timeout", kindCounter),
		TotalCounter:            r.registerCounter("accept.total", kindCounter),
		SuccessCounter:          r.registerCounter("accept.success", kindCounter),
		ErrorCounter:            r.registerCounter("accept.error", kindCounter),
		HandshakeTimeoutCounter: r.registerCounter("accept.timeout", kindCounter),
...
// registerCounter registers a monotonic prometheus.Counter and records its
// descriptor.
func (r *Registry) registerCounter(dotted string, kind legacyKind) Counter {
...
// registerGauge registers a prometheus.Gauge rendered as a gauge (".value") and
// returns the raw instrument so the caller can Set it.
func (r *Registry) registerGauge(dotted string, isFloat bool) prometheus.Gauge {
	...
	r.addDescriptor(&descriptor{dotted: dotted, promName: fqName(r.namespace, dotted), kind: kindGauge, isFloat: isFloat})
	return g
}

// registerOpenGauge registers conn.open: a prometheus.Gauge (so it can be
// decremented) rendered as a counter (".count") to preserve its historical
// single-value output.
...
// single is a counter/gauge reading in the normalized snapshot.
type single struct {
	dotted  string
	kind    legacyKind
	isFloat bool
...
func readSingle(d *descriptor, fam *dto.MetricFamily) single {
	v := single{dotted: d.dotted, kind: d.kind, isFloat: d.isFloat}
	...
	case dto.MetricType_COUNTER:
		v.f = m0.GetCounter().GetValue()
	case dto.MetricType_GAUGE:
		v.f = m0.GetGauge().GetValue()
	}
	v.i = int64(v.f)
	return v
}
