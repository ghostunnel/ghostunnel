...
// Logger is the minimal logging surface the push reporters use.
...
// prefix-prepended dotted name. Value types match the historical output exactly
// (int64 for counts/min/max, float64 for mean/percentiles and float gauges) so
// the encoded JSON is byte-compatible.
		...
		for range ticker.C {
			if err := r.postOnce(url, client); err != nil && err != io.EOF {
				if logger != nil {
					...
					log.Printf("error reporting metrics: %s", err)
				}
			}
		}
	}()
