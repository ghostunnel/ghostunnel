...
| `--allow-pin PIN` | Allow clients matching the given base64-encoded SHA-256 SPKI pin (repeatable). Out-of-band key pinning; chain, validity, and hostname are not verified. Mutually exclusive with other access control flags. |
...
| `--verify-pin PIN` | Verify the server matches the given base64-encoded SHA-256 SPKI pin (repeatable). Out-of-band key pinning; unlike other verify flags, hostname/chain/validity checks are not performed. Mutually exclusive with other verification flags. |
