...
> auto-detected). The **\--cert** and **\--key** flags can be used to
> load a certificate chain and key from separate PEM files (instead of a
> combined one).
...
> **\--pkcs11-pin** to configure the HSM. PKCS#11 options can also be
> set via environment variables (**PKCS11_MODULE**,
...
**\--allow-uri**, or **\--allow-policy**. All checks are made against
the certificate of the client. Multiple flags are treated as a logical
...
verification flags (**\--verify-cn**, **\--verify-ou**,
**\--verify-dns**, **\--verify-uri**).
...
**\--cacert=CACERT** Path to CA bundle file (PEM/X509). Uses system
...
**\--max-concurrent-conns=0** Maximum number of concurrent connections
to handle in the proxy. Zero means infinite.

**\--metrics-graphite=ADDR** Collect metrics and report them to the
given graphite instance (raw TCP).
...
**\--pkcs11-token-label=LABEL** Token label for slot/key in PKCS11
module (optional).
...
**\--target=ADDR** Address to forward connections to (can be HOST:PORT
or unix:PATH).

**\--target-status=\"\"** Address to target for status checking
downstream healthchecks. Defaults to a TCP healthcheck if this flag is
not passed.
...
**\--allow-ou=OU** Allow clients with given organizational unit name
(can be repeated).
...
Specify the URL to the ACME CA\'s Test/Staging environment. If set, all
requests will go to this CA and \--auto-acme-ca will be ignored.
...
**\--target=ADDR** Address to forward connections to (must be
HOST:PORT).
...
CONNECT or SOCKS5). Must be a proxy URL.
